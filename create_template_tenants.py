...
for unit_id, unit_number, rent_amount, property_name in occupied_units:
    try:
        # Generate template tenant data
        tenant_number = f"T{str(tenant_count + 1001).zfill(4)}"
        
        # Create tenant user account
        user_id = str(uuid.uuid4())
        email = f"tenant.{unit_number.lower().replace(' ', '')}@template.com"
        ...
        print(f"   ✅ {property_name} - Unit {unit_number}: {tenant_number}")
