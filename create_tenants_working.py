...
cur.execute("""
    SELECT u.id, u."unitNumber", u."rentAmount", u."depositAmount", p.name
    FROM units u
...
for unit_id, unit_number, rent_amount, deposit_amount, property_name in occupied_units:
    try:
        ...
        # Create active lease agreement (NO createdById field!)
        lease_id = str(uuid.uuid4())
        
        cur.execute("""
            INSERT INTO lease_agreements (
                id, "unitId", "tenantId", "startDate", "endDate",
                "rentAmount", "depositAmount", status,
                "createdAt", "updatedAt"
            )
