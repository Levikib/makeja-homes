      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="bg-gray-800/50 border border-gray-700 rounded-xl p-6">
          ...
          <div className="space-y-2 text-sm">
            ...
            {tenants[0] && <div><span className="text-gray-400">Tenant:</span> <span className="text-white ml-2">{tenants[0].users?.firstName} {tenants[0].users?.lastName}</span></div>}
          </div>
        </div>
        ...
        <div className="bg-gray-800/50 border border-gray-700 rounded-xl p-6">
          ...
          <div className="space-y-2 text-sm">
            ...
            {assignedTo ? (
              <div><span className="text-gray-400">Assigned to:</span> <span className="text-white ml-2">{assignedTo.firstName} {assignedTo.lastName}</span></div>
            ) : (
        ...
        <div className="bg-gray-800/50 border border-gray-700 rounded-xl p-6">
          <h2 className="text-lg font-semibold text-white mb-3">Timeline & Cost</h2>
          <div className="space-y-2 text-sm">
            ...
            {req.estimatedCost != null && <div><span className="text-gray-400">Est. Cost:</span> <span className="text-white ml-2">KSH {Number(req.estimatedCost).toLocaleString()}</span></div>}
            {req.actualCost != null && <div><span className="text-gray-400">Actual Cost:</span> <span className="text-white ml-2">KSH {Number(req.actualCost).toLocaleString()}</span></div>}
          </div>
        </div>
      </div>
