...
export default function NewPurchaseOrderClient({ properties, inventoryItems }: NewPurchaseOrderClientProps) {
  ...
  const updateLineItem = (index: number, field: string, value: any) => {
    ...
    if (field === "itemId") {
      ...
      if (item) {
        ...
        updated[index].unitCost = item.unitCost;
        updated[index].total = updated[index].quantity * item.unitCost;
      }
    } else if (field === "quantity") {
      updated[index].quantity = parseFloat(value) || 0;
      updated[index].total = updated[index].quantity * updated[index].unitCost;
    } else if (field === "unitCost") {
      updated[index].unitCost = parseFloat(value) || 0;
      updated[index].total = updated[index].quantity * updated[index].unitCost;
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!supplier || !propertyId || lineItems.length === 0) {
    ...
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-gray-400 mt-1">Create a new purchase order</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="bg-gray-800/50 border border-gray-700 rounded-xl p-6 space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
              <label className="block text-sm font-medium text-gray-300 mb-2">
                Expected Delivery <span className="text-gray-500">(Optional)</span>
              </label>
              <input
                type="date"
                ...
                <div key={index} className="bg-gray-900/50 border border-gray-700 rounded-lg p-4">
                  <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                    <div className="md:col-span-2">
                      ...
                      <select
                        ...
                        className="w-full px-4 py-2 bg-gray-800 border border-gray-600 rounded-lg text-white focus:ring-2 focus:ring-blue-500 focus:outline-none"
                      >
                        {inventoryItems.map((invItem) => (
                          <option key={invItem.id} value={invItem.id}>
                            {invItem.name} ({invItem.unitOfMeasure})
                          </option>
                        ))}
                      </select>
                    </div>
                  ...
                  <div className="flex items-center justify-between mt-4 pt-4 border-t border-gray-700">
                    <div className="text-sm">
                      ...
                      <span className="text-white font-bold">KSH {item.total.toLocaleString()}</span>
                    </div>
                    <button
                      type="button"
                      onClick={() => removeLineItem(index)}
                      className="flex items-center gap-1 px-3 py-1 text-sm text-red-400 hover:bg-red-500/10 rounded transition-colors"
                    >
                      <X className="w-4 h-4" />
                      Remove
                    </button>
        ...
        {lineItems.length > 0 && (
          <div className="bg-blue-500/10 border border-blue-500/30 rounded-xl p-6">
            <div className="flex items-center justify-between">
              ...
              <p className="text-3xl font-bold text-blue-400">KSH {totalAmount.toLocaleString()}</p>
            </div>
          </div>
        )}
        ...
        <div className="flex justify-end gap-4">
          ...
          <button
            ...
            className="px-4 py-2 bg-gradient-to-r from-blue-600 to-cyan-600 text-white rounded-lg hover:from-blue-700 hover:to-cyan-700 transition-colors disabled:opacity-50 flex items-center gap-2"
          >
