...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    try {
      ...
      await tenantPrisma.$executeRawUnsafe(`
        CREATE TABLE IF NOT EXISTS "${s}"."payments" (
          ...
          "paymentType" "${s}"."PaymentType" NOT NULL DEFAULT 'RENT',
          "paymentMethod" "${s}"."PaymentMethod" NOT NULL DEFAULT 'CASH',
      ...
      await tenantPrisma.$executeRawUnsafe(`
        CREATE TABLE IF NOT EXISTS "${s}"."water_readings" (
          ...
          "unitsConsumed" DOUBLE PRECISION NOT NULL DEFAULT 0,
    ...
    const patchSql = [
      ...
      `ALTER TABLE "${schemaName}"."properties" ADD COLUMN IF NOT EXISTS "contractTemplate" TEXT`,
      ...
      // payments — align old schemas to new column names used by all API routes
      `ALTER TABLE "${schemaName}"."payments" ADD COLUMN IF NOT EXISTS "paymentType" "${schemaName}"."PaymentType" NOT NULL DEFAULT 'RENT'`,
      `ALTER TABLE "${schemaName}"."payments" ADD COLUMN IF NOT EXISTS "paymentMethod" "${schemaName}"."PaymentMethod" NOT NULL DEFAULT 'CASH'`,
      `ALTER TABLE "${schemaName}"."payments" ADD COLUMN IF NOT EXISTS "referenceNumber" TEXT`,
      `ALTER TABLE "${schemaName}"."payments" ADD COLUMN IF NOT EXISTS "paystackReference" TEXT`,
      `ALTER TABLE "${schemaName}"."payments" ADD COLUMN IF NOT EXISTS "paystackStatus" TEXT`,
      // water_readings — align column names used by tenant bills API
      `ALTER TABLE "${schemaName}"."water_readings" ADD COLUMN IF NOT EXISTS "tenantId" TEXT`,
